...
This is the exact database schema used by the backend. It includes all tables, indexes, and Drizzle relations. Use it as the source of truth for recreating the DB layer.

**Source**
- `db/schema.ts`

**DB Connection**
- Uses Neon Postgres via `@neondatabase/serverless` with Drizzle.
- See `db/drizzle.ts` at the end of this document.
